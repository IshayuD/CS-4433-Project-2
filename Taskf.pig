...
int, PersonID:
int, myFriend:
int, DateofFriendship: chararray,
...
int, ByWho:
int, WhatPage:
int, TypeOfAccess: chararray,
 AccessTime: long);


-- Extract the PIDs from both datasets and remove duplicates
distinctRelationships =
DISTINCT (FOREACH relationship
 Generate PersonID, myFriend);
distinctAccesslogs =
DISTINCT (FOREACH accessLogs  Generate
 ByWho, WhatPage);
...
people = FOREACH (GROUP difference BY (PersonID,Name)) GENERATE group,Name;
